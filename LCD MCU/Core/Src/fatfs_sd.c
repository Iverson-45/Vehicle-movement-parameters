...
// W PEŁNI BEZPIECZNE FUNKCJE SPI - zapobiegają błędom Overrun (OVR)
...
{
    uint8_t dummy;
    // Używamy TransmitReceive zamiast Transmit, by czyścić sprzętowy bufor RX!
    HAL_SPI_TransmitReceive(HSPI_SDCARD, &data, &dummy, 1, 10);
}
...
static uint8_t SD_ReadBlock(BYTE *buffer, UINT length)
{
    ...
    // Klasyczna pętla bajt po bajcie, którą miałeś w oryginale
    while(length--)
    {
...
static uint8_t SD_WriteBlock(const uint8_t *buffer, BYTE token)
{
    ...
    if (token != 0xFD)
    {
        // Klasyczna pętla bajt po bajcie, zamiast błędnego HAL_SPI_Transmit bloku
        for(int i = 0; i < 512; i++)
        {
            SPI_TransmitByte(buffer[i]);
        }
