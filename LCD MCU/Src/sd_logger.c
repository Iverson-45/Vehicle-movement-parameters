...
uint8_t Logger_StartNewFile(void)
{
    ...
    /* Manual filename construction to avoid sprintf */
    for (int i = 0; i < 99; i++)
    {
    ...
    if (f_open(&logFile, fileName, FA_WRITE | FA_CREATE_ALWAYS) == FR_OK)
    {
        ...
        /* Manual header write to avoid sprintf overhead */
...
void Logger_AddData(TelemetryData_t *data)
{
    ...
    float relativeTime = (HAL_GetTick() - recordingStartTime) * 0.001;

    char valBuf[16];
    char *ptr = &dataBuffer[bufferIndex];

    /* Manual CSV construction to remove sprintf dependency */
...
void Logger_FlushBuffer(void)
{
    if (isFileOpen && !isCardEjected)
    {
        ...
        if (bufferIndex > 0)
        {
            res = f_write(&logFile, dataBuffer, bufferIndex, &bytesWritten);
            bufferIndex = 0;
