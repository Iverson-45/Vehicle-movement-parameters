...
int main(void)
{
    ...
    UART_SendString(USART1, "Measurement MCU: Booted.\n\r");
    ...
    uint8_t tick_counter = 0; // Licznik Non-Blocking dla UART
    ...
    while (1)
    {
        ...
        /* IMU Data Ready (Ticking at ~100Hz) */
        if (bmi160_data_ready)
        {
            ...
            tick_counter++; // Rośnie o 1 co 10ms
            ...
            switch (current_flow)
            {
                ...
                case CAL_STATE_WAIT_START_CMD:
                    // Wysyłaj 'ack' co 200ms (20 ticków) bez opóźniania procesora!
                ...
                case CAL_STATE_WAIT_STEP1_CONFIRM:
                    // Spamu co 200ms dopóki LCD nie potwierdzi
                    if (tick_counter % 20 == 0) {
...
// ---------------------------------------------------------
// KRYTYCZNA POPRAWKA ISR: Zapobiega zawieszaniu (ORE Clear)
// ---------------------------------------------------------
void USART6_IRQHandler(void)
{
    if (USART6->SR & USART_SR_RXNE)
    {
        ...
        if (rx_data == 'd') lcd_ack_state = 1;
        if (rx_data == '1') lcd_ack_state = 2;
        if (rx_data == '2') lcd_ack_state = 3;
        if (rx_data == 'k') lcd_ack_state = 4;
    }

    // Zabezpieczenie przed Overrun Error (ORE)
    if (USART6->SR & (USART_SR_ORE | USART_SR_NE | USART_SR_FE | USART_SR_PE))
    {
        volatile uint32_t tmpreg = USART6->DR; // Odczyt rejestru czyści błąd
        (void)tmpreg;
    }
}
